...
grid.newpage()
g <- draw.pairwise.venn(area1 = 44008, area2= 456598, cross.area = 488, 
                   category = c("State 1", " C001UY-Hypermethylation"),
                   # lty = rep("blank", 2),
                   fill = c("red", "orange"), 
                   alpha = rep(0.5, 2), 
                   cex = 1.5, # Numeros
...
# A - Por segmentos
seg <- read.table(file = "./solap_seg.txt", 
...
ggplot(seg, aes(x=seg[, 1], y=seg[, 3]*100)) +
  ...
  labs(x = "Minimun overlapping fraction (f)", y = "overlapping segments (%)",
       title = "Evolution of overlapping with the minimun overlapping fraction (Hypermethylation)", fontsize = 12 )

# dev.off()
ggsave(file = "Estudio_f_seg_hyper.pdf")


# B - por pares de bases
# A - Por segmentos
pb <- read.table(file = "./solap_pb.txt", 
...
ggsave(file = "Estudio_f_pb_hyper.pdf")
```
